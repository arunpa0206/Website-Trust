...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
  ...
  class Radium_Theme_Importer {
    ...
    public function __construct() {
      ...
      $this->give_form          = $this->demo_files_path . $this->give_form_file_name;
    ...
    public function process_imports() {
      ...
      if ( !empty( $this->give_form ) && is_file( $this->give_form ) ) {
        $this->set_demo_data_give_form( $this->give_form );
      }
    ...
    public function set_demo_data_give_form( $file ) {
      // File exists?
      if ( ! file_exists( $file ) ) {
        return;
      }
          ...
          foreach ($data as $table => $content) {
              ...
              if (is_array($content)) {
                  foreach ($content as $row) {
                      $v_temp = '';
                      if (is_array($row)) {
                          $row = $this->replace_url($row);
                          foreach ($row as $value) {
                              $v_temp .= ($v_temp ? ',' : '') . "'" . $value . "'";
                          }
                      }
                      $values .= ($values ? ',' : '') . '(' . $v_temp . ')';
                  }
              }

              if (count($wpdb->get_results($wpdb->prepare("SHOW TABLES LIKE %s", $wpdb->prefix . $table), ARRAY_A)) > 0) {
