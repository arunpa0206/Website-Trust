...
function give_razorpay_process_payment( $donation_data ) {
	...
	if ( empty( $razorpay_response['error_code '] ) ) {
		...
		if ( ! empty( $razorpay_response['razorpay_order_id'] ) ) {
			...
			give_insert_payment_note( $payment, sprintf( __( 'Transaction Successful. Razorpay Transaction ID: %s', 'give-razorpay' ), $razorpay_response['razorpay_order_id'] ) );
		...
		} elseif ( ! empty( $razorpay_response['razorpay_subscription_id'] ) ) {
			/**
			 * This action hook is used to complete the recurring donation as the subscription is successful.
			 *
			 * @since 1.3.0
			 */
			do_action( 'give_razorpay_complete_recurring_donation', $razorpay_response, $donation_data );
		}
	} else {
