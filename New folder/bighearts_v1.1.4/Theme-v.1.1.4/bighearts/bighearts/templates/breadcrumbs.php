...
if (!class_exists('BigHearts_Breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
    ...
    class BigHearts_Breadcrumbs
    {
        ...
        {
            $this->text_home     = esc_html__('Home', 'bighearts'); // text for the `Home` link
            $this->text_category = esc_html__('Archive by Category "%s"', 'bighearts'); // text for a category page
            $this->text_search   = esc_html__('Search Results for "%s"', 'bighearts'); // text for a search results page
            $this->text_tag      = esc_html__('Posts Tagged "%s"', 'bighearts'); // text for a tag page
            $this->text_author   = esc_html__('Articles Posted by %s', 'bighearts'); // text for an author page
            $this->text_404      = esc_html__('Error 404', 'bighearts'); // text for the 404 page
            $this->text_page     = esc_html__('Page %s', 'bighearts'); // text 'Page N'
            $this->text_cpage    = esc_html__('Comment Page %s', 'bighearts'); // text `Comment Page N`
        }
        ...
        public function __construct ()
        {
            ...
            if ( is_home() || is_front_page() ) {
	            ...
	            if( class_exists( 'WooCommerce' ) && ( is_product() || is_product_category() || is_product_tag() || is_cart() || is_checkout())){
		            ...
		            $out .= bighearts_woocommerce_breadcrumb($args);
                    ...
                    if ( get_query_var('paged') ) {
                        ...
                        if ($show_current) {
                            $out .= $sep . $before . sprintf($this->text_category, single_cat_title('', false)) . $after;
                        }
                    }
                } elseif ( is_search() ) {
                    if (have_posts()) {
                    ...
                    if ($show_current) {
                        $out .= $sep . $before . get_the_time('F') . $after;
                    }
                } elseif ( is_year() ) {
                    ...
                    if ('post' !== get_post_type()) {
                        ...
                        if (get_query_var('cpage')) {
                            $out .= $sep
                                . sprintf($link, esc_url(get_permalink()), get_the_title())
                                . $sep
                                . $before
                                . sprintf($this->text_cpage, get_query_var('cpage'))
                                . $after;
                        } else {
                            if ($show_current) {
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(BigHearts_Theme_Helper::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'bighearts')) . $sep
                            . $before
                            . ($is_category ? sprintf($this->text_category, $tax_name) : sprintf($this->text_tag, $tax_name))
                            . ', ' . sprintf($this->text_page, get_query_var('paged'))
                            . $after;
                    } else {
                        if ($show_current) $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'bighearts')) . $sep
                            . $before
                            . ($is_category ? sprintf($this->text_category, $tax_name) : sprintf($this->text_tag, $tax_name))
                            . $after;
                    }
                ...
                // Custom Post Type
                    ...
                    && is_product()
                    && function_exists('bighearts_woocommerce_breadcrumb')
                ) {
                    $args = [];
                    $args['delimiter'] = $sep;
                    if ($show_home_link) $out .= $sep;
            ...
            echo BigHearts_Theme_Helper::render_html($out);
        }
    }

    new BigHearts_Breadcrumbs();
}
